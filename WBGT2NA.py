...
for y in range(Start_year,End_year+1):
    ...
    for month in range(Start_month,End_month+1):
        ...
        WBGT_globe=xr.load_dataset(f)
        time=WBGT_globe.coords['time'].values#Varn[0]
        lat=WBGT_globe.coords['latitude'].values
        lon=WBGT_globe.coords['longitude'].values
        WBGT_globe=WBGT_globe['WBGT']
        #WBGT_globe=Varn[1]
        #lat=lat[:]
        #lon=lon[:]
